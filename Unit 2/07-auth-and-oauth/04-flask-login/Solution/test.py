...
class TestUser(TestCase):
    ...
    def test_user_registeration(self):
        ...
        with self.client:
            ...
            self.assertEqual(current_user.username, "tigarcia")
            # make sure we hash the password!
            self.assertNotEqual(current_user.password, "secret")
    ...
    def test_current_user(self):
        ...
        with self.client:
            ...
            self.assertEqual(current_user.username, "eschoppik")
    ...
    def test_add_messages(self):
        ...
        with self.client:
            ...
            response = self.client.post('/users/1/messages/', data=dict(content='Hello World',user_id=current_user.id), follow_redirects=True)
    ...
    def test_edit_messages(self):
        ...
        with self.client:
            self._login_user('eschoppik','secret')
            self.client.post('/users/1/messages/', data=dict(content='Hello World',user_id=current_user.id), follow_redirects=True)
            response = self.client.post('/users/1/messages/1?_method=PATCH' , data= dict(
                content="Hello Everyone", user_id=current_user.id
            ), follow_redirects=True)
            ...
            self.assertTrue(message.user_id == current_user.id)
    ...
    def test_cannot_modify_another_users_message(self):
        ...
        with self.client:
            ...
            response = self.client.post('/users/1/messages/1?_method=PATCH' , data= dict(
                text="Here are updated comments.", user_id=current_user.id
            ), follow_redirects=True)
