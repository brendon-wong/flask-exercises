...
- I have implemented the part 2 solution with Flask-Dance instead because Flask-OAuth is currently unmaintained
    ...
    - If using Flask-Migrate with Flask-Dance, the alembic migration script must be edited to `import sqlalchemy_utils` to successfully create the token column provided by Flask_Dance's `OAuthConsumerMixin`
