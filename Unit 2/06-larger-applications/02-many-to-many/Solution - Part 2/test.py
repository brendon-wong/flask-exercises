...
class BaseTestCase(TestCase):
    ...
    def test_messages_index(self):
        ...
        # Test tag in messages index
        self.assertIn(b'Greeting', response.data)
    ...
    def test_tags_index(self):
        response = self.client.get(
            '/tags', content_type='html/text', follow_redirects=True)
        self.assertLess(response.status_code, 400)
        self.assertIn(b'Greeting', response.data)
        self.assertIn(b'Valediction', response.data)
        # Test message in tags index
        self.assertIn(b'Elie', response.data)
    ...
    def test_tags_create(self):
        ...
        self.assertIn(b'Tag Created!', response.data)
    ...
    def test_tags_update(self):
        response = self.client.patch(
            '/tags/1?_method=PATCH',
            data=dict(name="Updated"),
            follow_redirects=True
        )
        self.assertIn(b'Updated', response.data)
        self.assertNotIn(b'Greeting', response.data)
